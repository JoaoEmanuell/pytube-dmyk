...
from .request import filesize, seq_filesize, stream, seq_stream
...
class Stream:
    ...
    def __init__(self, stream: Dict, monostate: Monostate):
        ...
        self.mime_type, self.codecs = mime_type_codec(stream["mimeType"])
    ...
    def filesize(self) -> int:
        ...
        if self._filesize == 0:
            try:
                self._filesize = filesize(self.url)
            except HTTPError as e:
                if e.code != 404:
                    raise
                self._filesize = seq_filesize(self.url)
    ...
    def filesize_kb(self) -> float:
        ...
        if self._filesize_kb == 0:
            try:
                self._filesize_kb = float(ceil(filesize(self.url) / 1024 * 1000) / 1000)
            except HTTPError as e:
                if e.code != 404:
                    raise
                self._filesize_kb = float(
                    ceil(seq_filesize(self.url) / 1024 * 1000) / 1000
                )
        ...
        if self._filesize_mb == 0:
            try:
                self._filesize_mb = float(
                    ceil(filesize(self.url) / 1024 / 1024 * 1000) / 1000
                )
            except HTTPError as e:
                if e.code != 404:
                    raise
                self._filesize_mb = float(
                    ceil(seq_filesize(self.url) / 1024 / 1024 * 1000) / 1000
                )
        ...
        if self._filesize_gb == 0:
            try:
                self._filesize_gb = float(
                    ceil(filesize(self.url) / 1024 / 1024 / 1024 * 1000) / 1000
                )
            except HTTPError as e:
                if e.code != 404:
                    raise
                self._filesize_gb = float(
                    ceil(seq_filesize(self.url) / 1024 / 1024 / 1024 * 1000) / 1000
                )
    ...
    def download(
        ...
        with open(file_path, "wb") as fh:
            try:
                for chunk in stream(self.url, timeout=timeout, max_retries=max_retries):
